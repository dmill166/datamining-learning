...
class DataTransformerV2:
    ...
    def __init__(self) -> None:
        self.db_path = os.getenv("DATABASE_PATH", os.path.join("data", "enterprise_education.duckdb"))
        self.conn = duckdb.connect(self.db_path)
        logger.info("[ENTERPRISE] DuckDB connected: %s", self.db_path)
    ...
    def load_grade3_to_silver(self, records: list[dict]) -> None:
        ...
        if "all_students_flag" not in df.columns:
            df["all_students_flag"] = True # Default for Census/HPS-level joins
    ...
    def load_income_to_silver(self, raw_data: list) -> None:
        """Enterprise-grade normalization for Median Household Income."""
        if not raw_data:
            logger.warning("[ENTERPRISE] Empty income data. Skipping.")
            return
        ...
        # Enterprise Audit
        self._upsert_table(df, "silver_income")
        logger.info("[ENTERPRISE] Loaded %d rows → silver_income", len(df))
